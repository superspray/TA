...
from ta_util import inputvalidator
from ta_util import apply
...
class TA:
    ...
    def Wilder(cls, data: Series, periods : int):
   ...
   @classmethod
    def PVO(
        ...
        """
        pvo = cls.PPO(ohlcv, period_fast, period_slow, column = 'volume')   
    ...
    def VO(
        ...
        (12-day EMA of Volume - 26-day EMA of Volume)
        """

        SMA_fast = ohlcv.volume.rolling(window=period_fast).mean()
        SMA_slow = ohlcv.volume.rolling(window=period_slow).mean()
        return pd.Series(((SMA_fast - SMA_slow) / SMA_slow) * 100, name = "VO")
    ...
    def TR(cls, ohlc: DataFrame) -> Series:
        ...
        _TR["TR"] = _TR.max(axis=1, skipna = False)
    ...
    def DMI(cls, ohlc: DataFrame, period: int = 14, adjust: bool = True) -> DataFrame:
        ...
        ## Smoothed
        TR1 = cls.TR(ohlc)
        TR14 = cls.Wilder(TR1, 14)

        pos14 = cls.Wilder(pos, 14)
        neg14 = cls.Wilder(neg, 14)
        ...
        return pd.concat(
            [   pd.Series(pos_di, name="DI+"),
                pd.Series(neg_di, name="DI-"),
                pd.Series(dx, name="DX"),
                pd.Series(dmi, name="DMI"),
            ],
            axis=1,
        )
    ...
    def ADX(cls, ohlc: DataFrame, period: int = 14, adjust: bool = True) -> Series:
        ...
        dx = cls.DMI(ohlc, period).DX
        adx = cls.Wilder(dx, 14)
    ...
    @classmethod
    def ADXR(cls, ohlc: DataFrame, period: int = 25) -> DataFrame:
        """
        """

        ohlc["up_move"] = ohlc["high"].diff().clip(lower = 0)
        ohlc["down_move"] = -ohlc["low"].diff().clip(lower = 0)
        up = ohlc["high"].diff().clip(lower = 0)
        dn = -ohlc["low"].diff().clip(lower = 0)
        ...
        ## Smoothed
        TR1 = cls.TR(ohlc)
        TR14 = cls.smoothing(TR1, 14)

        pos1 = pos
        pos14 = cls.smoothing(pos1, 14)

        neg1 = neg
        neg14 = cls.smoothing(neg1, 14)
    ...
    @classmethod
    def MarketFI(cls, ohlc: DataFrame) -> DataFrame:
        """
        Market Facilitation Index
        """

        mfi = (ohlc.high - ohlc.low)/ohlc.volume

        return pd.Series(mfi, name="MFI")

    @classmethod
    def AROON(cls, ohlc: DataFrame, period: int = 25) -> DataFrame:
        """
        ...
        """
        aroon_up = 100 * ohlc.high.rolling(window = period+1, min_periods=period+1).apply(lambda x: x.argmax()) / period

        aroon_down = 100 * ohlc.low.rolling(window = period+1, min_periods=period+1).apply(lambda x: x.argmin()) / period

        return pd.DataFrame({'aroonup':aroon_up, 'aroondown':aroon_down})


    @classmethod
    def AROONOSC(cls, ohlc: DataFrame, period: int = 25) -> Series:
        """
        The Aroon indicators fluctuate above/below a centerline (50) and are bound between 0 and 100. These three levels are important for interpretation. At its most basic, the bulls have the edge when Aroon-Up is above 50 and Aroon-Down is below 50. This indicates a greater propensity for new x-day highs than lows. The converse is true for a downtrend. The bears have the edge when Aroon-Up is below 50 and Aroon-Down is above 50.
        """
        ar = cls.AROON_test(cls,ohlc, period)
        aro = ar.aroonup - ar.aroondown
        return pd.Series(aro, name="ARO")


    @classmethod
    @inputvalidator(input_="ohlcv")

    def VHF(cls, ohlcv: DataFrame, period: int = 28) -> Series:
        """
        Vertical Horizontal Filter
        """
        highest = ohlcv.close.rolling(window = period, min_periods=period).max() 
        lowest = ohlcv.close.rolling(window = period, min_periods=period).min() 
        close_dif = ohlcv.close.diff().abs()
        den = close_dif.rolling(window = period).sum()
        return pd.Series((highest - lowest)/den, name = "VHF")


    @classmethod

    def CV(cls, ohlc: DataFrame, period: int = 10) -> Series:
        """
        Chaikin Volatility
        ...
        """
        hla = (ohlc["close"] - ohlc["low"]).ewm(span = 10, min_periods= 9).mean()
        vol = hla.diff(n)/hla.shift(n)*100
        return pd.Series(vol, name = "CV")

    @classmethod
    def Disparity(cls, ohlc: DataFrame, period: int = 14, column: str = "close") -> Series:
        """
        """
        ma = ohlc[column].rolling(window=period).mean()
        disp = (ohlc[column] - ma) / ma *100
        return pd.Series(disp, name="{0} period Disparity".format(period),
        )

    @classmethod
    def DPO(cls, ohlc: DataFrame, period: int = 20) -> Series:
        """
    ...
    def PVR(cls,
        ...
        def _pvrank(x):
            if pd.isna(x.close) | pd.isna(x.volume):
                return np.nan
            if x.close == 1:
                if x.volume == 1:
                    return 1
    ...
    @classmethod
    def PSY(cls,
        ohlcv: DataFrame, period : int = 14
    ) -> Series:
        """
        The Psychological Line indicator
        """

        mv_close = ohlc.close.diff().apply(lambda x: 1 if x>0 else (np.nan if pd.isna(x) else 0) )
        psy = mv_close.rolling(period).mean() * 100
        return pd.Series(psy, name = "PSY")
